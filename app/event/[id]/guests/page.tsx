...
export default function GuestsPage() {
  ...
  const getTransportCount = (optionName: string) => {
    return guests.reduce((sum: number, g: any) => {
      const t = (g.transportation || g.transport || '').toString();
      if (!t.includes(optionName)) return sum;
      return sum + getGuestQty(g);
    }, 0);
  };
  ...
  const countSeparation = () => {
    ...
    guests.forEach((g: any) => {
      ...
      const mm = sep.match(/(\d+)\s*גבר/);
      const ww = sep.match(/(\d+)\s*(?:אישה|אשה|נשים|אישות)/);
      if (mm) men += parseInt(mm[1], 10);
      if (ww) women += parseInt(ww[1], 10);
    });
    return { men, women };
  };
