...
export default function SeatingPage() {
  ...
  const [guests, setGuests] = useState<string[]>(INITIAL_GUESTS);
  ...
  useEffect(() => {
    ...
    const savedGuests = localStorage.getItem('seatingGuests');
  ...
  useEffect(() => {
    localStorage.setItem('seatingGuests', JSON.stringify(guests));
  }, [guests]);
  ...
  return (
    <div className="flex h-screen bg-slate-900 text-white flex-col" dir="rtl">
      <div className="bg-slate-800 p-4 flex items-center gap-4 border-b border-slate-700 z-50">
        ...
        <div className="flex-1 max-w-md">
          <input type="text" placeholder="חיפוש מוזמנים שהושבו..." value={assignedSearch} onChange={(e) => setAssignedSearch(e.target.value)}
            className="w-full bg-slate-700 border border-slate-600 rounded-2xl px-5 py-3 focus:outline-none focus:border-emerald-500" />
        </div>

        <button onClick={returnAllGuests} className="bg-amber-600 hover:bg-amber-700 px-6 py-3 rounded-3xl whitespace-nowrap">חזרה לרשימת המוזמנים</button>
        <button onClick={() => { if (confirm('לאפס סקיצה?')) { setPlacedTables([]); setGuests(INITIAL_GUESTS); setSelectedId(null); setSelectedGuests(new Set()); setSelectedSeatedGuests(new Set()); } }} className="bg-red-600 hover:bg-red-700 px-6 py-3 rounded-3xl">אפס סקיצה</button>
      </div>
      ...
      <div className="flex flex-1 overflow-hidden">
            ...
            <div className="space-y-2 max-h-[45vh] overflow-y-auto pr-2">
              {filteredUnassigned.map((guest, i) => (
                <div key={i} className="bg-slate-700 p-3 rounded-xl flex items-center gap-3 hover:bg-slate-600">
                  <input 
                    type="checkbox" 
                    checked={selectedGuests.has(guest)} 
            ...
            {showTableTypes && (
              <div className="space-y-3 mb-10">
                {TABLE_TYPES.map((item, i) => (
                  <div key={i} onClick={() => addRegularTable(item)} className="bg-slate-700 hover:bg-slate-600 p-4 rounded-2xl cursor-pointer flex items-center gap-4 text-sm transition-all active:scale-95">
        ...
        <div className="flex-1 relative overflow-auto" onDrop={onDropTable} onDragOver={(e) => e.preventDefault()}
          ...
          {placedTables.map(table => (
            ...
            >
              ...
              {!table.isSpecial && Array.from({ length: table.seats }).map((_, i) => {
                ...
                return (
                  <div key={i} className={`absolute w-7 h-8 border-2 rounded-md flex items-center justify-center text-xs shadow-md transition-all ${isOccupied ? 'bg-red-600 border-red-800 scale-110' : 'bg-amber-50 border-amber-900'}`}
              ...
              {!table.isSpecial && (table.type === 'rect' || table.seats >= 20) && (
                <button 
                  ...
                  className="absolute -top-3 -left-3 bg-blue-600 hover:bg-blue-700 text-white w-7 h-7 rounded-full flex items-center justify-center text-xs shadow-lg z-30"
                >
                  ↻
                </button>
              )}
          ...
          {hoveredTable && hoveredTable.assignedGuests.length > 0 && (
            <div className="absolute bg-black/90 text-white text-sm p-4 rounded-2xl shadow-2xl pointer-events-none z-50 max-w-xs" 
              style={{ left: hoveredTable.x + 120, top: hoveredTable.y - 90 }}>
                ...
                <div className="space-y-2 mb-6">
                  {selectedTable.assignedGuests.map((g, i) => (
                    <div key={i} className="bg-slate-700 p-3 rounded-xl flex items-center gap-3">
                      <input 
                        type="checkbox" 
                        checked={selectedSeatedGuests.has(g)} 
                        onChange={() => toggleSeatedGuestSelection(g)}
          ...
          <div className="bg-white text-black rounded-3xl p-8 w-96">
            ...
            <div className="space-y-6">
              <div>
                ...
                <input type="number" value={editingTable.tableNumber || ''} onChange={(e) => setEditingTable({...editingTable, tableNumber: parseInt(e.target.value)})} className="w-full p-3 border rounded-xl" />
              </div>
              <div>
