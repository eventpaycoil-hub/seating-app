...
export default function SeatingPlanner() {
  const GRID_SIZE = 60; // הקטנו את הרצפה
  ...
  const handleDragStartFromFloor = (e: React.DragEvent, tableId: number) => {
    e.dataTransfer.setData('tableId', tableId.toString());
    e.dataTransfer.setData('isNew', 'false');
  };

  const handleDragStartGuest = (e: React.DragEvent, guestName: string) => {
  ...
  return (
    <div className="flex h-screen bg-[#f8f5f0] text-[#333] overflow-hidden">
      ...
      <div className="w-80 bg-white border-l border-gray-200 p-4 overflow-y-auto flex-shrink-0">
        ...
        <div className="mb-8">
          <h3 className="text-sm font-semibold text-gray-500 mb-3">שולחנות בסיסיים</h3>
          <div className="grid grid-cols-2 gap-3">
            {standardTables.map((table, index) => (
              <div
                key={index}
                draggable
                onDragStart={(e) => handleDragStartFromSidebar(e, table)}
                className="group flex flex-col items-center gap-2 p-4 bg-white border border-gray-200 rounded-2xl cursor-grab active:cursor-grabbing hover:border-[#d4b88a] hover:shadow transition-all text-center"
              >
                <div className="text-5xl text-[#3c2f1e] group-hover:scale-110 transition-transform">
                  {table.icon}
                </div>
                <div className="font-semibold text-sm">{table.label}</div>
                {table.seats > 0 && <div className="text-xs text-gray-500">{table.seats} מושבים</div>}
              </div>
            ))}
          </div>
        </div>
        ...
        {customTables.length > 0 && (
          <div className="mb-8 border-2 border-blue-400 rounded-3xl p-4 bg-blue-50">
            <div 
              onClick={() => setShowNewTables(!showNewTables)}
              className="flex justify-between items-center cursor-pointer mb-3"
            >
              <h3 className="font-semibold text-blue-600">שולחנות חדשים שנוספו ({customTables.length})</h3>
              <span className="text-xl text-blue-600">{showNewTables ? '▲' : '▼'}</span>
            </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                {customTables.map((table) => (
                  <div
                    ...
                    draggable
                    onDragStart={(e) => handleDragStartFromSidebar(e, table)}
                    className="group flex flex-col items-center gap-2 p-4 bg-white border border-blue-300 rounded-2xl cursor-grab active:cursor-grabbing hover:border-blue-500 hover:shadow transition-all text-center"
                  >
                    <div className="text-5xl text-[#3c2f1e] group-hover:scale-110 transition-transform">
                      {table.shape === 'עגול' ? '⚪' : '⬛'}
                    </div>
                    <div className="font-semibold text-sm text-center">{table.label}</div>
                    <div className="text-xs text-gray-500">{table.seats} מושבים</div>
                  </div>
                ))}
              </div>
            )}
                ...
                {table && !isSecondCell && (
                  <div className="w-full h-full flex flex-col items-center justify-center relative">
                    ...
                    <div className={`text-3xl mb-1 ${table.type === 'knights24' ? 'text-4xl' : ''}`}>
                      {table.label ? table.label[0] : standardTables.find(t => t.type === table.type)?.icon}
                    </div>
                    
                    <div className="flex gap-0.5 text-[6px] text-[#3c2f1e] opacity-80 flex-wrap justify-center">
                      {Array.from({ length: Math.floor((standardTables.find(t => t.type === table.type)?.seats || 12) / 4) }).map((_, i) => (
