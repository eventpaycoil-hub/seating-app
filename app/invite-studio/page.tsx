...
export default function InviteStudioPage() {
  ...
  const publicLink =
    typeof window !== 'undefined' && inviteId
  ...
  const save = async () => {
    ...
    const payload = {
      ...
      quote: form.quote,
      date: form.date,
      hebrew_date: form.hebrewDate,
      reception_time: form.receptionTime,
      chuppah_time: form.chuppahTime,
      hall_name: form.hallName,
      city: form.city,
      welcome_line: form.welcomeLine,
      invite_line: form.inviteLine,
      groom_parents: form.groomParents,
      bride_parents: form.brideParents,
    ...
    localStorage.setItem(
      `invitation_standalone_${inviteId}`,
      JSON.stringify({ id: inviteId, ...form, updatedAt: payload.updated_at })
    );
    ...
    try {
      ...
      if (error) {
        console.warn(error);
        alert('נשמר במכשיר, אבל הענן נכשל: ' + error.message);
        return;
      }
    } catch (e) {
      console.warn(e);
      alert('נשמר במכשיר, סנכרון ענן נכשל');
      return;
    }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="space-y-5">
                ...
                {TEMPLATES.map((t) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="bg-white px-3 py-2 text-xs font-semibold border-t">
                        {t.name}{active ? ' · נבחר ✓' : ''}
                      </div>
                    </button>
                  );
            ...
            <div className="bg-white rounded-3xl border shadow-sm p-5 space-y-2">
              <div className="font-bold mb-2">2. הפרטים שלכם</div>
              ...
              <input className="w-full border rounded-xl px-3 py-2.5" placeholder="ציטוט" value={form.quote} onChange={(e) => set('quote', e.target.value)} />
              ...
              <div className="grid grid-cols-2 gap-2">
                <input className="w-full border rounded-xl px-3 py-2.5" placeholder="תאריך" value={form.date} onChange={(e) => set('date', e.target.value)} />
              ...
              <div className="flex flex-wrap gap-2 pt-3">
                ...
                {inviteId && (
                  <Link href={`/i/${inviteId}`} target="_blank" className="bg-blue-600 text-white px-5 py-2.5 rounded-xl font-bold">
                    תצוגה ↗
                  </Link>
                )}
              </div>
          ...
          <div className="bg-white rounded-3xl border shadow-sm p-5 h-fit lg:sticky lg:top-4">
