...
function parseGenderCounts(sep: string): { men: number; women: number } {
  ...
  if (s === 'גבר') return { men: 1, women: 0 };
  if (s === 'אישה' || s === 'אשה' || s === 'נשים') return { men: 0, women: 1 };
  if (s === 'זוג') return { men: 1, women: 1 };

  let men = 0;
  let women = 0;
  const menMatch = s.match(/(\d+)\s*גבר/);
  const womenMatch = s.match(/(\d+)\s*(?:אישה|אשה|נשים|אישות)/);
  if (menMatch) men = parseInt(menMatch[1], 10);
  if (womenMatch) women = parseInt(womenMatch[1], 10);
  return { men, women };
}
...
export default function EditGuestPage() {
  ...
  useEffect(() => {
    ...
    if (foundGuest) {
      ...
      const isNumber =
        status && !isNaN(Number(status)) && Number(status) >= 1 && Number(status) <= 16;
      ...
      const simpleValues = ['גבר', 'אישה', 'אשה', 'זוג', 'נשים'];
      if (sep && !simpleValues.includes(sep)) {
        setGenderMode('custom');
        const { men, women } = parseGenderCounts(sep);
        setMenCount(men);
        setWomenCount(women);
      } else {
